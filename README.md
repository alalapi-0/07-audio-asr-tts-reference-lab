...
`check_mcp_config.py` / `check_mcp_config.js` 检查 `.cursor/mcp.json` 是否存在、JSON 是否合法、是否包含必需的 5 个 MCP server，以及 filesystem 是否有过宽授权。详见 `docs/agent_skills/mcp_usage_skill.md`。

## Workspace MCP Servers

当前项目需要以下 Workspace MCP（配置在 `.cursor/mcp.json`）：

- **chrome-devtools** — 浏览器调试、console、network、页面状态
- **context7** — 第三方库与框架文档查询
- **filesystem** — 仅当前项目目录（`${workspaceFolder}`）
- **github** — 仓库、提交、分支、issue、PR（需环境变量 token）
- **playwright** — 浏览器自动化与 E2E 验收

说明：

1. `.cursor/mcp.json` 是本项目的 Workspace MCP 配置。
2. Cursor 可能需要**重启**或 **Reload Window** 后才能识别新 server。
3. GitHub MCP 通过环境变量提供 token（如 `GITHUB_TOKEN` → `${env:GITHUB_TOKEN}`），**不得**写进仓库。
4. filesystem MCP 只授权当前项目目录，不得授权 `/`、`~`、整盘 `/Volumes` 等大范围路径。
5. 可运行 `npm run check:mcp` 检查配置（需 Node.js）。
