...
- **github**：仓库、提交、分支、issue、PR 等相关操作（token 仅通过环境变量，不得写入仓库）。
- **playwright**：浏览器自动化、页面操作、E2E 检查。

自动推进轮开始前，Agent 必须确认上述 MCP 已在 Cursor Settings → MCP 中加载（修改配置后通常需 **Reload Window** 或重启 Cursor）。可运行 `npm run check:mcp` 或 `node scripts/check_mcp_config.js` 做轻量校验。

若某个 MCP 不可用，Agent 需要记录原因，并按 `docs/agent_skills/mcp_usage_skill.md` 使用可用替代方案继续推进，不要无意义反复安装。

涉及页面、审核台、生成结果、预览、发布流程的任务，必须使用 **chrome-devtools** 或 **playwright** 进行真实浏览器检查，不能仅凭代码推断成功。

- **文件操作** 必须确认真实磁盘状态（filesystem MCP 或 Read/Write/Grep）。
- **GitHub 操作** 必须在提交前检查 `git diff`，避免泄露密钥、真实音频或原文。
- 若需要 token / API Key 但当前环境没有，则进入 mock / dry-run，不要卡死整体流程。

详细说明见 `docs/agent_skills/mcp_usage_skill.md`、`.cursor/rules/mcp-agent-tools.mdc` 与 `.cursor/rules/mcp-required.mdc`。
